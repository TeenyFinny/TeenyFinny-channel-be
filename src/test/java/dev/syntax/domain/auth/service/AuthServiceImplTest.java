...
import static dev.syntax.global.response.error.ErrorAuthCode.*;
...
class AuthServiceImplTest {
	...
	void verifyIdentity_success() {
		...
		System.out.println("birthBack=[" + req.birthBack() + "]");
		// when
		IdentityVerifyRes res = authService.verifyIdentityMock(req);
	...
	void verifyIdentity_invalid_birth_format() {
		...
		// when & then
		assertThatThrownBy(() -> authService.verifyIdentityMock(req))
